...
namespace Nifti.NET
{
    public static class NiftiFile
    {
        ...
        /// <returns></returns>
        public static Nifti Read(string path)
        {
            ...
            using (var stream = ReadStream(path))
            {
                ...
                {
                    result.Data = ReadData(stream, hdr);
                }
            }
            ...
            if(FileType.HDR == TypeOf(result.Header))
            {
                ...
                if(File.Exists(imgpath))
                {
                    ...
                    {
                        result.Data = ReadData(stream, result.Header);
                    }
                }
            }
        ...
        private static dynamic ReadData(Stream stream, NiftiHeader hdr)
        {
            var reverseBytes = hdr.SourceIsBigEndian();
            var bytelen = stream.Length - stream.Position;
            var data = InitData(hdr.datatype, bytelen);
